...
```{python}
# Additional step: 
# Convert ZIP_CD to string for future use
...
# Number of directly affected ZIP codes
...
# Use the renamed columns to filter indirectly affected ZIP codes
# Filter out any ZIP codes that are directly affected
...
indirectly_affected_zip_num = indirectly_affected_zips['ZCTA5_left'].nunique()# attribute: here I have error for ZCTA5, but I did not realize it, so I use chatgpt to debug here.
...
```{python}
# Start by initializing all ZIP codes as "Not Affected"
...
# Plotting the choropleth map with three categories
texas_zip_codes.plot(column='affected_status', legend=True).set_axis_off()

```

## Reflecting on the exercise (10 pts) 
1. The following are the potential issues of the "first pass" method:
